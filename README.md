...
\rarrow{}        % מציג → (חץ ימינה) בכיוון נכון ב-RTL (במקום →)
$\neq$           % מציג ≠ (לא שווה) - השתמש ב-math mode
```

**הערה חשובה על חצים:** הפקודה `\rarrow{}` משתמשת ב-`\textenglish{$\rightarrow$}` כדי לוודא שהחץ מופיע בכיוון הנכון גם בטקסט RTL.
